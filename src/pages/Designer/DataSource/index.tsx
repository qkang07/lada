import { DataSourceRuntime, DataSourceSchema, DataSourceType } from '@/components/compDef'
import React, { useState } from 'react'
import styles from './index.module.less'
import { Button, Form, Input, Popover, Select, Space, Trigger } from '@arco-design/web-react'
...
const DataSources = (props: Props) => {
  ...
  return (
    <SidePane title="数据" actions={  [ <Popover 
      title="编辑数据源"
      position='rt'
      ...
      content={<div className={styles.newPop}>
        ...
        <div className={styles.dsEditFooter}>
          <Space align='end'>
            <Button type='text' size='small' onClick={()=> setNewVisible(false)}>取消</Button>
            <Button type='primary' size='small' onClick={saveNew}>保存</Button>
          </Space>
        </div>
      </div>}>
        <Button icon={<IconPlus/>} size='small' type='text' onClick={add}></Button>
      </Popover>]}>
      <div className={styles.dsList}>
        
        {schemas.map((ds, i) => {
          return <div key={i} className={styles.dataSourceItem}>
